...
- **Investigate first, via sub-agents by default.** Sub-agents are the default for research because they read in their own context window — your context stays clean for the source doc, the plan you're writing, and your own thinking. Use sub-agents for: exploring codebase areas, scanning long ancillary files, verifying technology claims against current sources, summarising prior tickets or docs. Direct reads (Read tool, no limit/offset) stay reserved for the source doc and any plan section you're filling in; skim-and-summarise on the spec is how context drifts, sub-agent on research is how context survives. Ask the user only what investigation can't answer; don't ask about things already written down.
...
2. **Write decisions to the plan doc before the next question.**
    - *Why:* the doc is the persistence layer; conversation is volatile. Decisions held across multiple turns evaporate at context reset, tab close, or session resume — and they evaporate without warning, since you don't know which turn will be the last. Worse, your next questions stack up against unrecorded prior decisions, the user has to keep correcting "we already settled that", and the conversation transcript drifts away from the doc until the doc is a lie about what's been agreed. The rhythm is: user decides → reflect back → write to doc → ask the next question. Not: decide → ask → decide → ask → eventually write a summary. If you've accumulated three settled decisions and made zero doc edits between them, you've already failed — recover by writing now, not after the next question.

3. **Don't modify the codebase.**
...
6. **Answer the question, don't hedge or pivot.**
    - *Why:* user questions ("what X?", "which Y?", "should we Z?", or ending in `?`) are requests for information. Three failure modes, all misreads of what the user actually asked for:
      ...
      - **Autonomy-creep.** Treating your own answer as the user's decision. After design options on the table, if the user's next message is a question, the next output is text only — no Edit, no Write, no tool call beyond read-only research — then stop. Wait for an explicit decision ("go with option 1", "use your pick", "write it up that way") before any plan-doc edit. Your recommendation, even one the user implicitly invited, is not a decision.
