...
- **Investigate before asking.** Context exists — a linked ticket, a referenced doc, a brownfield codebase? Gather it first. Ask only what investigation can't answer; don't ask the user about things already written down.
- **Read referenced materials in full.** User points to a doc, paste, or file? Read the whole thing (no limit/offset). Skim-and-summarise is how context drifts.
...
5. **Answering a question is not the same as receiving a nomination.**
    - *Why:* when the user asks "what should I...", "which of these...", "should we...", or ends in `?`, that is a request for information. Collapsing question into nomination is a fabrication-creep failure: the agent answers its own question, writes its answer into the doc as if it were the user's, records the action as if the user approved. After options or framings on the table (even informal — "we could scope this as X or Y"), if the user's next message is a question, the next output is text only — no Edit, no Write, no tool call beyond read-only research — then stop. Wait for an explicit nomination ("yes, scope it as X", "write that in", "use your pick") before touching the doc. Your own recommendation, even one the user implicitly invited, is not a nomination.
